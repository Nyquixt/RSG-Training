...
def main():
    ...
    test_parser = ChunkParser(FileDataSrc(test),
                              ...
                              sample=args.sample,
                              batch_size=RAM_BATCH_SIZE).parse()
    print('done with chunk parser, processing')

    tfprocess = TFProcess(blocks, filters)
    tfprocess.init(RAM_BATCH_SIZE, logbase=args.logbase, macrobatch=BATCH_SIZE // RAM_BATCH_SIZE)
    print('pass init')
    ...
    # THIS IS THE ONLY BUG WE HAVE LEFT!!!
