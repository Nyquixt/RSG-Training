...
import time
import unittest
from config import BOARD_SIZE, INTERSECTS, PROBS, LEARNING_RATE, MOMENTUM
...
class TFProcess:
    ...
    def init(self, batch_size, macrobatch=1, gpus_num=None, logbase='leelalogs'):
        ...
        probs = tf.reshape(probs, [batch_size, PROBS]) # 3328 values
    ...
    def init_net(self, planes, probs, winner, gpus_num):
        self.y_ = probs   # (tf.float32, [None, 26])
    ...
    def process(self, train_data, test_data): # MAIN
        info_steps=200
        stats = Stats()
        timer = Timer()
        while True:
            ...
            if steps % 1000 == 0: # write accuracy
                test_stats = Stats()
    ...
    def batch_norm(self, net): # batch normalization
    ...
    def conv_block(self, inputs, filter_size, input_channels, output_channels, name): # first conv block
    ...
    def residual_block(self, inputs, channels, name): # 2 conv layers per res block
        net = inputs
    ...
    """
        ...
        (1) Convo2D, 256 filters, kernel size = 3x3, stride 1
        (2) Batch Norm
        (3) ReLU

        Residual Tower, with each Residual Block:
        (1) Convo2D, 256 filters, kernel size = 3x3, stride 1
        (2) Batch Norm
        (3) ReLU
        (4) Convo2D, 256 filters, kernel size = 3x3, stride 1
        (5) Batch Norm
        ...
        Output: 2 'heads'
        (*) Policy Head
        (1) Convo2D, 2 filters, kernel size 1x1, stride 1
        (2) Batch Norm
        (3) ReLU
        (4) Dense, outputs a vector of size BOARDSIZE^2 + 1 (logit probability: all intersections + pass)

        (*) Value Head
        (1) Convo2D, 1 filter, kernel size 1x1, stride 1
        (2) Batch Norm
        (3) ReLU
        (4) Dense(256)
        (5) ReLU
        (6) Dense(1) -> scalar
        (7) Tanh outputs a scalar in range [-1, 1]
    """

    def construct_net(self, planes): # net construction
        # NCHW format
        ...
        W_fc1 = weight_variable("w_fc_1", [2 * BOARD_SIZE * BOARD_SIZE, PROBS], self.model_dtype)
        b_fc1 = bias_variable("b_fc_1", [PROBS], self.model_dtype)
...
class TFProcessTest(unittest.TestCase):
    def test_can_replace_weights(self):
        ...
        data.append([0.4] * 2*BOARD_SIZE*BOARD_SIZE * (PROBS))
        data.append([0.5] * (PROBS))
        # value
