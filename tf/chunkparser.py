...
import time
import unittest
from config import BOARD_SIZE, INTERSECTS, PROBS
...
def remap_vertex(vertex, symmetry):
    ...
    """
    assert vertex >= 0 and vertex < INTERSECTS
    x = vertex % BOARD_SIZE
    y = vertex // BOARD_SIZE
    if symmetry >= 4:
        x, y = y, x
...
class ChunkParser:
    def __init__(self, chunkdatasrc, shuffle_size=1, sample=1,
        ...
        self.prob_reflection_table = [
            [remap_vertex(vertex, sym)
              for vertex in range(INTERSECTS)]+[INTERSECTS] for sym in range(8)]
        # Build full 16-plane reflection tables.
        self.full_reflection_table = [
            np.array([remap_vertex(vertex, sym) + p * INTERSECTS
                for p in range(16) for vertex in range(INTERSECTS)])
                    for sym in range(8)]
        ...
        self.flat_planes = [ b'\1'*INTERSECTS + b'\0'*INTERSECTS, b'\0'*INTERSECTS + b'\1'*INTERSECTS ]
    ...
    def init_structs(self):
        ...
        # (5*5+1) float32 probabilities (1448 bytes) (104 for 5x5)
        # 5*5*16 packed bit planes (722 bytes) (50 for 5x5)
        # uint8 side_to_move (1 byte)
        # uint8 is_winner (1 byte)
        v2_struct_str = '4s' + str(PROBS * 4) + 's' + str(INTERSECTS * 2) + 'sBB'
        self.v2_struct = struct.Struct(v2_struct_str) # THIS MTFKER IS CRUCIAL

        # Struct used to return data from child workers.
        # float32 winner
        # float32*26 probs
        # uint*5*5*18 planes (6498 for 19x19; 450 for 5x5)
        # (order is to ensure that no padding is required to
        #  make float32 be 32-bit aligned)
        raw_struct_str = '4s' + str(PROBS * 4) + 's' + str(INTERSECTS * 18) + 's'
        self.raw_struct = struct.Struct(raw_struct_str) # THIS MTFKER IS CRUCIAL
    ...
    def convert_v1_to_v2(self, text_item):
        ...
        # each being a BOARDSIZE*BOARDSIZE == 25 element array
        # of type np.uint8
        planes = []
        for plane in range(0, 16):
            # first 24 first bits are 6 hex chars, encoded MSB
            hex_string = text_item[plane][0:int((BOARD_SIZE * BOARD_SIZE - 1) / 4)]
            try:
            ...
            last_digit = text_item[plane][int((BOARD_SIZE * BOARD_SIZE - 1) / 4)]
        ...
        # We flatten to a single array of len 16*BOARDSIZE*BOARDSIZE, type=np.uint8
        ...
        if np.any(np.isnan(probabilities)):
            ...
            return False, None
        if not(len(probabilities) == PROBS):
            return False, None

        probs = probabilities.tobytes()
        if not(len(probs) == PROBS * 4):
            return False, None
    ...
    def v2_apply_symmetry(self, symmetry, content):
        ...
        assert len(planes) == INTERSECTS*16
    ...
    def convert_v2_to_tuple(self, content):
        ...
        assert len(planes) == (18 * INTERSECTS), len(planes)
...
class ChunkParserTest(unittest.TestCase):
    def generate_fake_pos(self):
        """
            ...
            Result is ([[25] * 18], [26], [1])
        """
        # 1. 18 binary planes of length 25
        planes = [np.random.randint(2, size=INTERSECTS).tolist()
                  for plane in range(16)]
        stm = float(np.random.randint(2))
        planes.append([stm] * INTERSECTS)
        planes.append([1. - stm] * INTERSECTS)
        # 2. 26 probs
        probs = np.random.randint(3, size=PROBS).tolist()
    ...
    def test_parsing(self):
        ...
        items = []
        for p in range(16):
            # generate first 24 bits
            h = np.packbits([int(x) for x in planes[p][0:int(INTERSECTS-1)]]).tobytes().hex()
            # then add the stray single bit
            h += str(planes[p][int(INTERSECTS-1)]) + "\n"
            items.append(h)
        # then side to move
        ...
        batch = ( np.reshape(np.frombuffer(data[0], dtype=np.uint8),
                             (batch_size, 18, INTERSECTS)).tolist(),
                  np.reshape(np.frombuffer(data[1], dtype=np.float32),
                             (batch_size, PROBS)).tolist(),
            ...
            for symmetry in range(8):
                ...
                sym_planes = [
                    [plane[remap_vertex(vertex, symmetry)]
                        for vertex in range(INTERSECTS)]
                            for plane in planes]
                sym_probs = [
                    probs[remap_vertex(vertex, symmetry)]
                        for vertex in range(INTERSECTS)] + [probs[INTERSECTS]]
